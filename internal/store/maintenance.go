...
type RestoreResult struct {
	...
	Bytes  int64            `json:"bytes"`
	At     time.Time        `json:"at"`
	// UndecryptableKeys 是用当前 pepper 集无法解密密文的 Key 数（0 表示
	// 全部可解或库内本就没有带密文的 Key）。
	UndecryptableKeys int `json:"undecryptable_keys,omitempty"`
	// PepperWarning 是给管理面板展示的人读告警；仅在检测到不可解密时非空。
	PepperWarning string `json:"pepper_warning,omitempty"`
}
