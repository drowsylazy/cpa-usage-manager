...
- **flex/grid 嵌套的收缩链（2026-09 实锤，圆环图例占比被挤出）**：内层元素配了 min-width:0 + ellipsis 仍不截断时，先查外层——grid/flex 子项默认 min-width:auto 内容不可收缩，长文本会把整行撑破轨道、把同行的占比/数值列推出容器。修复从外层放开（该层加 min-width:0），内层截断才生效。面板内任何「名字列 + 数值列」的行布局都适用（.bar-cell、.donut-legend-item 均踩过）。
- **兜底计费必须与真实成本可能性对齐（2026-09 实锤，$7.05 天价扣费事故）**：上游（agentrouter）空响应致零用量，旧链路 missing_usage=settle_reserved 按「(body字节/2 + max_tokens) × 四档最贵档」的全额预占入账——22 万 token agent 请求被多扣 5 万倍（真实 $0.00012 → 扣 $7.05）。两层修复定型：①预占估算 body/3+1 + 分档计价（输入估算按 max(input, cache_read, cache_creation) 档、输出估算按输出价；ReservationRequest.EstimatedInput/EstimatedOutput 任一非零才走分档，全零兼容旧口径）；②Settle 第 5 参 noResponse——执行器判定「未产生任何响应数据」（HTTP≥400、空响应体、流一条块都没收到即 firstChunkAt.IsZero()）时零用量按**零成本结算**（不是 Release，请求行照常落库保留可观测性），有响应数据但缺 usage 才按预占入账（settle_reserved 防逃逸语义保留）。注意请求行 token 恒记真实值（零用量即 0），预占估算只进额度累计器。
- **学习型估算的两条铁律（2026-09 密度学习三轮实测定型）**：①**预测侧不可能有逐请求「接受带」**——预占时无从得知当前请求的真实 token（要结算才有），任何「按折算值反推指标再比对样本统计带」的校验都是同义反复（反推值恒等于样本统计值本身），从不拒绝任何请求；v0.8.8 的 MAD 相对带就是这么一层死代码，看着像防线实为空气。防御只能放两层：学习侧（样本过滤、漂移探测）+ 极宽绝对 sanity 带（[0.2×,5×] 锚粗估 fallback，只拦构成级错误）。②**组成突变（agent /compact 后摘要替代原始代码）的适应必须做在学习侧的近期窗口**——全窗中位数要新样本过半才拉动（实测占比 100%→50% 再慢慢爬到 70%）；且漂移基准不能锚全窗 MAD（新旧混合期 MAD 被撑大、带自解除永远检不出），要锚「漂移窗口之外」的较旧样本。范式：EstimateDensity = 旧样本中位数 ± max(MAD_old, 15%) 基准带 vs 近期 20 条中位数。
- **测试种子跨批次必须单调递增（时间戳+ID 双要素，2026-09 实锤）**：seedRows 类 helper 若每次调用各自以 now 为锚、ID 只按批内下标生成，后一批种子的时间戳比前一批早（RecentDensities 按时间倒序取样直接把「新样本」排除在窗口外）、ID 还与前一批撞车被 errDuplicateRequest 静默吞掉（返回 nil 不报错！）——「新流量没生效」先查种子到底落库没有（SELECT COUNT），别急着怀疑业务逻辑。修法：包级游标 seedRowLast/seedRowBatch，跨批次时间与 ID 都严格递增。
