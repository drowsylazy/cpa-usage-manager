...
- `input_tokens` 为混合密度封顶粗估（ASCII 字节/4 + 多字节/3），并在该模型已有 body_len÷input 样本时按近期采纳密度折算。密度估计带**组成漂移适应**（EstimateDensity）：基准锚在漂移窗口之外的较旧样本（中位数 + max(MAD, 15%) 带——锚全窗 MAD 会被新旧混合撑大、带自解除），近期 20 条的中位数漂出基准带时判定组成突变（agent compact 后摘要散文替代原始代码，实测密度 7.3→10.4 字节/token、占比 100%→50%），改跟近期窗口中位数——10 条新流量部分恢复、20 条完全接管，全窗中位数则要新样本过半才动。预测侧只保留极宽的绝对 sanity 带 [0.2×, 5×]（锚混合估算）拦构成级错误；旧版「按折算值反推等效密度再比对样本带」是同义反复（反推值恒等于样本密度），从不拒绝任何请求，已删。
