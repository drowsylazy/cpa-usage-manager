...
- `input_tokens` 为混合密度封顶粗估（ASCII 字节/4 + 多字节/3），并在该模型已有 body_len÷input 样本时按近期采纳密度折算。密度样本的分母是 requests 行落库时归一算好的 `context_tokens`（schema v18，inclusive 行 = input 原值、exclusive 行 = input+缓存读+写，历史行由迁移就地回填）——**不可**在 SQL 端按 `input+cache_read+cache_creation` 拼：宿主回填会把 OpenAI inclusive 行的 cache_read_tokens 填成 cached_tokens 的镜像值（input 已含缓存命中，再拼即双计），实测 glm-5.3 分母 1.99×、deepseek-v4-flash 1.92×，密度学成一半、预占虚高约一倍、「最近预占」实际占比恒 52%（=1/1.923）。密度估计带**组成漂移适应**（EstimateDensity）：基准锚在漂移窗口之外的较旧样本（中位数 + max(MAD, 15%) 带——锚全窗 MAD 会被新旧混合撑大、带自解除），近期 20 条的中位数漂出基准带时判定组成突变（agent compact 后摘要散文替代原始代码，实测密度 7.3→10.4 字节/token、占比 100%→50%），改跟近期窗口中位数——10 条新流量部分恢复、20 条完全接管，全窗中位数则要新样本过半才动。预测侧只保留极宽的绝对 sanity 带 [0.2×, 5×]（锚混合估算）拦构成级错误；旧版「按折算值反推等效密度再比对样本带」是同义反复（反推值恒等于样本密度），从不拒绝任何请求，已删。
...
- 预占金额的输入侧按该模型近期流量的缓存构成拆三档计价（新鲜×输入价 + 读×读价 + 写×写价，token 加权份额，60s 缓存）：agent 长对话 90%+ 上下文走缓存读（约 0.1× 价）时，最贵档全额口径把在途金额放大近十倍；样本 <3 条回退输入侧最贵档。份额偏差只影响在途预占观感，结算恒按真实 token 逐档计。预占金额恒折算成 USD 账本口径（CNY 规则按当前实时汇率，与结算同口径）。
