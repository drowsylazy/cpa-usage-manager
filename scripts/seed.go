...
func main() {
	...
	for i, fin := range []struct {
		...
		p := store.HoldReservationParams{
			ID: fin.id, KeyID: kid, CallerID: store.DefaultCallerID,
			Model: fin.model, HeldMicroUSD: fin.held, ReservedTokens: fin.tokens,
			ExpiresAt: created.Add(2 * time.Hour), Now: created,
		}
		if _, _, err := st.HoldReservation(ctx, p); err != nil {
			log.Printf("预占 %s 失败（忽略）：%v", p.ID, err)
			continue
		...
		if fin.ok {
			...
			log.Printf("释放 %s 失败（忽略）：%v", p.ID, err)
		}
	}
