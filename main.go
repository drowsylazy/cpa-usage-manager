	...
	return rpcRegistration{
		...
		Metadata: rpcMetadata{
			...
			ConfigFields: []rpcConfigField{
				...
				{Name: "backup.include_peppers", Type: "boolean", Description: "自动备份附带 .peppers 侧车（0600，跨机恢复免手工迁移 pepper；默认关闭）。"},
...
func managementRegistration() rpcManagementRegistration {
	...
	routes := []rpcManagementRoute{
		...
		{Method: "GET", Path: base + "/reservations/accuracy", Description: "按模型聚合的预占精度（P50/P95 实际占比）"},
